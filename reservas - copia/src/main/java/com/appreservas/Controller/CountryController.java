...
@Api(tags = "Country", value = "Service Web RESTFul de Countrys")
public class CountryController {
    ...
    @GetMapping("searchByFirstname/{namecountry}")
    @ApiOperation(value = "Buscar Country por namecountry", notes = "Métodos para encontrar un country por su respectivo namecountry")
    @ApiResponses({
            @ApiResponse(code=201,message = "Country encontrados"),
            @ApiResponse(code=404,message = "Country no encontrados")
    })
    public ResponseEntity<List<Country>> findByFirstName(@PathVariable("namecountry")String namecountry){
        try {
            List<Country>countrys=countryService.findByFirstname(namecountry);
            if(countrys.size()>0)
                return new ResponseEntity<List<Country>>(countrys,HttpStatus.OK);
            else
                return new ResponseEntity<List<Country>>(HttpStatus.NOT_FOUND);
